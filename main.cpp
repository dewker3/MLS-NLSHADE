...
int main(int argc, char **argv) {
  ...
  for(int benchmark=7; benchmark<8; benchmark++) {
  	switch (benchmark) {
        case 0:
            code = "000";
            code2="Basic";
    ...
    for (int i =0;i <10; i++) {
        ...
        g_optimize_fitness=Fx[benchmark][g_function_number];
        ...
        file_name << "MLS-NLSHADE_(" << code << ")_" << g_function_number << "_" << g_problem_size << ".txt";
        ...
        cout  << "\nbest = " << best_bsf_fitness << ", worst = " << worst_bsf_fitness << endl;
        cout  << "\nmedian = " << median_bsf_fitness << ", mean = " << mean_bsf_fitness << endl;
        ...
        filename_2 << "fitness_(" << code << ")_" << g_function_number << "_" << g_problem_size << ".txt";

        // Convert the stringstream to string and then use c_str() to pass it to ofstream
        std::ofstream myfile(filename_2.str().c_str());
        ...
        if (myfile.is_open()) {
            ...
            cout << "Data has been saved to fitness_results.txt" << endl;
        } else {
...
void print_results(string file_name) {
    ...
    if (fout.is_open()) {
        ...
        cout << "Error! cannot open output file " << file_name << "." << endl;
    }
